...
describe("wsNativeApi", () => {
  ...
  it("forwards project directory browsing to the project RPC", async () => {
    rpcClientMock.projects.browseDirectories.mockResolvedValue({
      currentPath: "/tmp",
      parentPath: "/",
      roots: [
        { label: "Current", path: "/repo" },
    ...
    const api = createWsNativeApi();
    await api.projects.browseDirectories({ path: "/tmp" });

    expect(rpcClientMock.projects.browseDirectories).toHaveBeenCalledWith({
      path: "/tmp",
    });
  });
  ...
  it("forwards web push notification methods to the notifications RPC", async () => {
    rpcClientMock.notifications.getWebPushConfig.mockResolvedValue({
      vapidPublicKey: "test-vapid-public-key",
    });
    rpcClientMock.notifications.upsertWebPushSubscription.mockResolvedValue(undefined);
    rpcClientMock.notifications.removeWebPushSubscription.mockResolvedValue(undefined);
    const { createWsNativeApi } = await import("./wsNativeApi");
    ...
    expect(rpcClientMock.notifications.getWebPushConfig).toHaveBeenCalledWith();
    expect(rpcClientMock.notifications.upsertWebPushSubscription).toHaveBeenCalledWith({
      endpoint: "https://push.example/subscription",
      expirationTime: null,
      keys: {
        p256dh: "p256dh-key",
    ...
    expect(rpcClientMock.notifications.removeWebPushSubscription).toHaveBeenCalledWith({
      endpoint: "https://push.example/subscription",
    });
  });
