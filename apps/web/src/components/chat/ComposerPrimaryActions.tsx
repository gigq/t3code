...
export const ComposerPrimaryActions = memo(function ComposerPrimaryActions({
  ...
  pendingAction,
  isRunning,
  interruptAriaLabel = "Stop generation",
  ...
  return (
    <button
      type={showAutoModeTimer ? "button" : "submit"}
      className={cn(
        "flex h-9 w-9 items-center justify-center rounded-full text-white transition-all duration-150 sm:h-8 sm:w-8",
        showAutoModeTimer || hasSendableContent
          ? "enabled:cursor-pointer hover:scale-105"
          : "disabled:pointer-events-none disabled:opacity-30 disabled:hover:scale-100",
        isAutoMode ? "bg-amber-500/90 hover:bg-amber-500" : "bg-primary/90 hover:bg-primary",
        !isAutoMode ? "text-primary-foreground" : "",
      )}
      disabled={isSendBusy || isConnecting || (!showAutoModeTimer && !hasSendableContent)}
      onClick={showAutoModeTimer ? onDisableAutoMode : undefined}
      aria-label={
        isConnecting
          ? "Connecting"
          : isPreparingWorktree
